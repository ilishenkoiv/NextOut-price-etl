...
begin
  ...
  if p.origin is null or p.dest is null or p.origin=p.dest or p.month is null
    ...
    or p.updated_at is null or jsonb_typeof(p_offers) is distinct from 'array'
    or not(direct_seen or any_seen) or (p.direct is null and p.any_stops is null) then raise exception 'invalid cell'; end if;
