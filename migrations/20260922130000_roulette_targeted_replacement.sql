...
-- Provider/network/429/error results never enter this branch. Compared against the owner's
-- 2026-09-22 pg_get_functiondef readback of the deployed collection_commit_roulette function.
begin;
...
create table if not exists public.roulette_pool_replacements(
  ...
  historical_rows_removed integer not null check(historical_rows_removed>=1),
  run_id text,
