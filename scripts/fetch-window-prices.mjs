...
import { catalogue } from './collection-planning.mjs';
...
//   • DESTINATIONS ARE NARROWED BY HISTORY: the TOP_DESTS (50) catalogue destinations that have
//     most often yielded a fare for THIS origin are checked every day. The remaining catalogue is
//     split into seven stable daily slices, so every destination is still checked once per week
//     without a monthly full-pass spike.
//
// WHERE IT WRITES
...
const windows = computeAllWindows(holidays, regions, planDate, { horizonMonths: HORIZON_MONTHS, weekendAround: process.env.WEEKEND_AROUND === '1' });
...
const allowedDests = new Set(catalogue(process.env.EXPANSION_WAVE || 0).map(d=>d.iata));
const allDests = [...new Set(priceRows.map((r) => r.dest))].filter(dest=>allowedDests.has(dest)&&dest!==ORIGIN).sort();

// ── History-ranked daily destination plan ─────────────────────────────────────────────────────
// Ask the TOP_DESTS destinations every day plus one stable seventh of everything else. The
// find-count is NOT stored anywhere new: window_prices IS the history — one row per fare found — so
// we just count its rows per dest.
const TOP_DESTS = Number(process.env.TOP_DESTS) || 50;
const fullSweepEnv = (process.env.FULL_SWEEP || '').trim();
const destinationMode = fullSweepEnv === '1' ? 'full' : fullSweepEnv === '0' ? 'top-only' : 'auto';
...
const sweepMode = destinationPlan.bootstrap ? 'FULL (bootstrap — no history yet)'
  : destinationMode === 'full' ? 'FULL (manual override)'
  : destinationMode === 'top-only' ? `TOP-${TOP_DESTS} only (manual override)`
  : `TOP-${destinationPlan.top.length} + weekly tail ${destinationPlan.slice + 1}/7 (${destinationPlan.tail.length})`;
