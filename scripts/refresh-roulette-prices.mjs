...
async function main() {
  ...
  for (let index = start; index < tickets.length; index += 1) {
    ...
    if (result.status === 'found') {
      ...
      const [patch] = withPriceProvenance([{ price: result.fare.price, transfers: result.fare.transfers, airline: result.fare.airline, updated_at: updatedAt, flight_type: ticket.flight_type }], 'offers');
